...
export const CountryPage = (): JSX.Element => {
    ...
    return (
        <main className="w-full h-screen flex items-center flex-col text-white max-lg:px-4">
            ...
            <section className="max-w-screen-xl w-full mt-10">
                ...
                <ul className="grid grid-cols-4 gap-4 mt-10 max-lg:grid-cols-1">
                    {bordersList.length ? bordersList.map((country, i)=>( <CountryCard key={i} name={country.commonName} countryCode={country.countryCode}/>)) : <p>This country has no borders.</p>}
                </ul>
            </section>
        </main>
    );
